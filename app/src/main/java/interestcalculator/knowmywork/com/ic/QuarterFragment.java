...
public class QuarterFragment extends Fragment {
    ...
    private void startCalculation(){
        ...
        if(startYear > year1) {
            if (l == 0) {
                k = (4 - j) + 1;
                m = (pref.getConstantDate() - day1) / (double)90;
                k += m;
            } else {
                k = (4 - j);
                m = ((3 - l) * 30 + (pref.getConstantDate() - day1)) / (double)90;
                k += m;
            }
            ...
            if((month1 + 1) <= 3){
                ...
                if(l == 0){
                    m = ((j - 1) * 90 + (day1 - pref.getConstantDate())) / (double)90;
                    k = m;
                }else{
                    k = j - 1;
                    m = ((l * 30) + (day1 - pref.getConstantDate())) / (double)90;
                    k += m;
                }
    ...
    // second part of compounding interest within the period of year of calculation
    private void endCalculation(){
        ...
        if(year2 > lastYear){
            ...
            if((month2 + 1) > 3){
                ...
                if(u != 0) {
                    s = ((3 - u) * 30 + (pref.getConstantDate() - day2)) / (double) 90;
                    t = 4 - s;
                }else{
                    s = (pref.getConstantDate() - day2) / (double) 90;
                    t = 4 - s;
                }
            }
        }else{
            s = ((u * 30) + (day2 - pref.getConstantDate())) / (double)90;
            t = v - 1 + s;
        }
    }
    ...
    public double compoundInterest(double principle,double rate, double t){
        double interest;
        interest = principle * Math.pow((1 + (rate / (double)400)), t);
        return interest;
    }
